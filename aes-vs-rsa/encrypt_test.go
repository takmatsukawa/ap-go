...
func BenchmarkRSADecryption(b *testing.B) {
	...
	publicKey := &privateKey.PublicKey
	md5hash := md5.New()
	cipherData, err := rsa.EncryptOAEP(md5hash, rand.Reader, publicKey, sourceData, label)
	if err != nil {
		panic(err)
	}
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		rsa.DecryptOAEP(md5hash, rand.Reader, privateKey, cipherData, label)
	}
}
...
func BenchmarkAESDecryption(b *testing.B) {
	sourceData, nonce, gcm := prepareAES()
	cipherData := gcm.Seal(nil, nonce, sourceData, nil)
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		gcm.Open(nil, nonce, cipherData, nil)
	}
}
